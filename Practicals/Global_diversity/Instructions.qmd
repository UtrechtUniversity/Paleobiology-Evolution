...
If you're doing it in Python, you need to download the data through [the web interface](https://paleobiodb.org/data1.2/occs/list.csv?base_name=Pteridophyta&taxon_reso=species&max_ma=540&min_ma=0). Rename the file to `Pteridophyta_occurrences.csv` and place it in the project folder.
...
There is no dedicated package for manipulating PBDB data in Python but here's a function that does nearly the same thing. 

```{python}
# First you need to deal with the fact that some records are not at the genus level. This function extracts genus names from species names.

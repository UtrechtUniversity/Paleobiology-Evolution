---
title: "Global plant diversity (Python)"
...
```{python}
import pandas as pd
Pteridophyta = pd.read_csv("data/Pteridophyta_occurrences.csv")
```
...
In the Download page you can select the type of data you want to download. To get diversity data for Pteridophyta at the genus level and the temporal resolution of a geochronological age, use [this link](https://paleobiodb.org/data1.2/occs/diversity.csv?base_name=Pteridophyta&count=genera). Save the resulting file as `pbdb_data.csv` in the `data` folder where you are carrying out this analysis.
**Do not open it in Excel** as this program can distort some information.

```{python}
Pteridophyta_div = pd.read_csv("data/pbdb_data.csv")
```
